...
def create_heatpmap(strings: list):
    ...
    for s in strings:
        ...
        # dprint(commit_num, ': ', year, '/', month, '/', day) # debug
    ...
    # dprint(first_sunday) # debug
    date = first_sunday
    dprint('       Aug     Sep     Oct       Nov     Dec       Jan     Feb     Mar     Apr       May     Jub     Jul   ') # todo ハリボテいつか直す
    for i in range(7):
        dprint(f'\033[48;2;{10};{14};{18}m', end='') # 背景色を設定
        dprint(f'\033[37m{wdays[i]}\033[30m', end='')
        for j in range(WEEK_NUM):
            r, g, b = get_color(daily_commits.get(date, 0))
            dprint(f'\033[38;2;{r};{g};{b}m■ \033[30m', end='')
            date += dt.timedelta(7)
        dprint('\033[0m', end='') # 背景色の設定を解除
        dprint()
        date += dt.timedelta(-7 * WEEK_NUM + 1) # 7足しすぎているから(WEEK_NUM - 1)ではなくWEEK_NUM
    # dprint('\033[38;2;0;0;0mtest000\033[38;2;255;255;255mtest111\033[30m') # debug test
    ...
    #         # dprint(daily_commits.get(date, 0), end=' ') # debug
    #         date += dt.timedelta(-1)
    #     # dprint() # debug
