...
from yow.forWin.mylib.calendar.calendar import Calendar

from yow.forWin.mylib.heatmap.heatmap import create_heatpmap
from yow.forWin.mylib.heatmap.git import get_commits_per_date
from yow.forWin.mylib.heatmap.config import settings as heatmap_settings

from yow.forWin.mylib.linguist.linguist import create_linguist
from yow.forWin.mylib.linguist.config import settings as linguist_settings

from yow.forWin.mylib.abstinence.abstinence import create_abstinence

from yow.forWin.mylib.integrate.integrate import create_integrated

from yow.forWin.mylib import svg2png
from yow.forWin.mylib import change_wallpaper



chdir(path.abspath(path.join('yow', 'forWin')))
work_dir = getcwd()
...
for repository_path in heatmap_settings['paths']: # todo 例外処理
    daily_commits = get_commits_per_date(repository_path)
    create_heatpmap(daily_commits, path.join(tmp_dir, 'heatmap.svg'))
